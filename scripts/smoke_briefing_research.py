...
def run_javascript(research: dict) -> dict:
    harness = r"""
const fs = require('fs');
const input = JSON.parse(fs.readFileSync(0, 'utf8'));
const filterSource = fs.readFileSync(process.argv[1], 'utf8');
const buildSource = fs.readFileSync(process.argv[2], 'utf8');
const staticData = {};
const init = {
...
process.stdout.write(JSON.stringify({
  ...
  mail: {
    ...
    textPreview: mail.text.slice(0, 1600),
  },
}, null, 2));
"""
    result = subprocess.run(
        ["node", "-e", harness, str(FILTER_PATH), str(BUILD_PATH)],
        input=json.dumps(research, ensure_ascii=False),
        capture_output=True,
        text=True,
        encoding="utf-8",
        check=True,
